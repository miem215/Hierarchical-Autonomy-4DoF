...
import matplotlib.pyplot as plt  # Added for 3D plotting
...
def main():
    ...
    # --- NEW: Data logging arrays for the plot ---
    log_ee_pos = []
    log_obs_pos = []
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            # --- NEW: Log the data at this timestep ---
            ...
            if distance_to_final < tolerance:
                ...
                break  # Exit the simulation loop to trigger plotting
    ...
    # --- NEW: Generate the 3D Trajectory Plot ---
    print("📊 Generating 3D Trajectory Plot...")
    
    # Convert logs to numpy arrays for slicing
    log_ee = np.array(log_ee_pos)
    log_obs = np.array(log_obs_pos)
    
    # Setup publication-style plot
    plt.style.use('seaborn-v0_8-whitegrid' if 'seaborn-v0_8-whitegrid' in plt.style.available else 'default')
    fig = plt.figure(figsize=(10, 8), dpi=300)
    ax = fig.add_subplot(111, projection='3d')
    
    # 1. Plot the Global RRT* Reference Path
    ax.plot(waypoints[:, 0], waypoints[:, 1], waypoints[:, 2], 
            color='#bdc3c7', linestyle='--', linewidth=2, label='Global RRT* Reference')
    
    # 2. Plot the Actual NMPC Executed Trajectory
    ax.plot(log_ee[:, 0], log_ee[:, 1], log_ee[:, 2], 
            color='#2ecc71', linestyle='-', linewidth=3, label='NMPC Executed Trajectory')
    
    # 3. Plot the Dynamic Obstacle's Swept Path
    ...
    # Formatting
    ax.set_title("Hierarchical Autonomy: 3D Trajectory Execution in MuJoCo", fontsize=14, pad=20)
    ...
    # Adjust view angle for best visibility (isometric-ish)
    ...
    plt.tight_layout()
    plt.savefig('actual_mujoco_trajectory.png', bbox_inches='tight')
    plt.show()
